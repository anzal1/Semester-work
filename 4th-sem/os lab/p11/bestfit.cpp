...
void bestFit(Process p[], int n, Memory mem[], int m)
{
    int j = 0;
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < m; j++)
        {
            if (mem[j].size >= p[i].size)
            {
                if (best_index == -1)
                    best_index = j;
                else if (mem[best_index].size > mem[j].size)
                    best_index = j;
            }
        }
        ...
        if (best_index != -1)
        {
            mem[best_index].isTaken = true;
            p[i].isGiven = true;
            mem[best_index].givenProcessId = p[i].id;
            mem[best_index].free -= p[i].size;
            mem[best_index].size -= p[i].size;
            p[i].allocation = best_index + 1;
            mem[best_index].allocated = p[i].id;
